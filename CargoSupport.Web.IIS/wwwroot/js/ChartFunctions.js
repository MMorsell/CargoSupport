...
function getDataBetweenDates(apiUrl) {
  $.ajax({
    ...
    data: {
      fromDate: function () {
        return fromDate.format(timeFormat);
      },
      toDate: function () {
        return toDate.format(timeFormat);
      },
      splitData: function () {
        return splitData();
      },
...
function colorBasedOnLoadingValue(thisRef) {
  switch (thisRef.value) {
    case "Ej påbörjad":
      thisRef.style.backgroundColor = "red";
      break;
    case "Återanvända":
      thisRef.style.backgroundColor = "orange";
      break;
    case "Påbörjad":
      thisRef.style.backgroundColor = "yellow";
      break;
    case "Klar":
      thisRef.style.backgroundColor = "lightgreen";
      break;
  }
...
function generateTabs() {
  const tabDomRef = document.getElementById("dataTableTabs");
  $(tabDomRef).empty();
  ...
  for (let i = 0; i < tabs.length; i++) {
    if (tabs[i] !== "Morgon" && tabs[i] !== "Kväll" && tabs[i] !== "Hämtas" && tabs[i] !== "Returer") {
